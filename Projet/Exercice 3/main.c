...
int nbreChiffre(int nb);

int main(int argc, char* argv[])
{
    FILE* fichierMatrice = NULL;
    int* matrice = NULL;
    int ordreMatrice, nbreElemntMatrice, tailleColonne = 0, status;
    pid_t** pid_tTab = NULL;
    int i, j, a, b;
...
//Lecture de l'ordre de la matrice    
    ...
    nbreElemntMatrice = ordreMatrice * ordreMatrice;

//Création de la matrice de calcul en mémoire partagée
    keyMatrice = ftok("shmMatrice", 19);
    shmidMatrice = shmget(keyMatrice, 2 * nbreElemntMatrice * sizeof(int), 0666|IPC_CREAT);
...
//Lecture de la matrice dans le fichier et remplissage de la matrice de calcul    
    for (i = 0; i < ordreMatrice; i++) {
        for (j = 0; j< ordreMatrice; j++) {
            ...
            if (tailleColonne < nbreChiffre(matrice[i*ordreMatrice + j]))
                tailleColonne = nbreChiffre(matrice[i*ordreMatrice + j]);
...
//Création des chaines pour la transmission des paramètres aux processus    
    ...
    chO = malloc((1 + nbreChiffre(ordreMatrice)) * sizeof(char));
    ...
    chI = malloc((1 + nbreChiffre(ordreMatrice)) * sizeof(char));
    ...
    chJ = malloc((1 + nbreChiffre(ordreMatrice)) * sizeof(char));
...
//Arret des processus et du programme
    if (getpid() > 0) {
        ...
        for (i = 0; i < nbreElemntMatrice; i++) {
            if (tailleColonne < nbreChiffre(matrice[i+nbreElemntMatrice]))
                tailleColonne = nbreChiffre(matrice[i+nbreElemntMatrice]);
        }
...
//Affichage de la matrice de calcul
    for (i = 0; i < ordreMatrice; i++) {
        ...
        for (j = 0; j < ordreMatrice; j++) {
            printcol(tailleColonne, matrice[i*ordreMatrice + j + nbreElemntMatrice]);
            putchar('|');
        }
        printf("\n");       
    }
...
int printcol(const int colWidth, const int nb)
{
    const int strWidth = nbreChiffre(nb);
    int nbrSpace;
