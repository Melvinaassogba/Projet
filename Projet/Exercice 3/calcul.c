...
{
    int ordreMatrice = atoi(argv[1]), nbreElemntMatrice;
    int i = atoi(argv[2]);
    int j = atoi(argv[3]);
    int a, b;
    ...
    shmidMatrice = shmget(keyMatrice, 2 * nbreElemntMatrice * sizeof(int), 0666);
...
//Calcul du c[i,j] et enregistrement du c[i,j]
    matrice[i*ordreMatrice + j + nbreElemntMatrice] = 0;
    for (a = 0; a < ordreMatrice; a++) {
        matrice[i*ordreMatrice + j + nbreElemntMatrice] += tabI[a] * tabJ[a];  
    }
    printf("%d %d\n", i*ordreMatrice+j, matrice[i*ordreMatrice + j + nbreElemntMatrice]);
